...
// Uncomment below and remove mock data when DB is connected:
// $conn = getConnection();
...
/*
   LIVE DB QUERIES — uncomment when connected:
...
// ── MOCK DATA (remove once DB is live) ────────────────────────
$totalAll     = 0;
$totalSenior  = 0;
$totalPWD     = 0;
$totalChronic = 0;
$mockPatients = []; // replace with DB result
// ──────────────────────────────────────────────────────────────
...
<style>
/* ══════════════════════════════════════════
   BLUE THEME — mirrors dashboard structure
══════════════════════════════════════════ */
:root {
    /* Sidebar keeps the same forest-dark feel */
    ...
    /* Blue accent palette (replaces green in content area) */
    --blue-deepest:   #061528;
    --blue-dark:      #0d2744;
    --blue-mid:       #1a4a7a;
    --blue-rich:      #1e5c9c;
...
.page-icon {
    ...
    font-size: 24px;
    box-shadow: 0 4px 16px rgba(45,125,200,.35);
}
.topbar-left h1 {
    font-family: 'Playfair Display', serif;
...
.date-badge {
    ...
    border: 1px solid rgba(58,156,224,.2);
...
/* ══════════ STAT CARDS (matches dashboard cards exactly) ══════════ */
.cards {
    display: grid;
...
.card-1::after { background: var(--blue-leaf); }
.card-2::after { background: var(--amber); }
.card-3::after { background: #a855f7; }
.card-4::after { background: #ef4444; }
.card-icon {
    width: 44px; height: 44px; border-radius: 12px;
    display: flex; align-items: center; justify-content: center;
    font-size: 20px; margin-bottom: 16px;
}
.card-1 .card-icon { background: rgba(58,156,224,.12); }
.card-2 .card-icon { background: rgba(217,119,6,.12); }
.card-3 .card-icon { background: rgba(168,85,247,.12); }
.card-4 .card-icon { background: rgba(239,68,68,.12); }
.card h3 {
    font-family: 'Playfair Display', serif;
...
.btn-search-go {
    ...
    box-shadow: 0 3px 10px rgba(26,74,122,.3);
}
...
.patient-id {
    ...
    background: var(--blue-fog); padding: 2px 8px; border-radius: 20px;
}
.badge {
...
<aside class="sidebar">
    ...
    <nav class="nav-section">
        ...
        <a href="/health_monitoring/barangay_health/index.php">
            <span class="nav-icon">📊</span>Dashboard
        </a>
        <a href="/health_monitoring/patients/index.php" class="active">
...
<main class="main">
    ...
    <form method="GET" action="/health_monitoring/patients/index.php" class="toolbar">
        ...
        <span class="results-count">
            <!-- Replace 0 with $patients->num_rows when DB is live -->
            <strong>0</strong> patients found
        </span>
    </form>
    ...
    <div class="table-card">
        ...
        <div class="table-responsive">
            <table>
                ...
                /*
                 * ── LIVE DATA LOOP ───────────────────────────────────────
                 * Uncomment the block below and remove the empty-state
                 * placeholder once the DB connection is active.
                 *
                 ...
                 *             <a href="/health_monitoring/patients/delete.php?id=<?= $pid ?>" class="btn-action btn-delete"
                ...
                <?php // endif; /* close the DB result check */ ?>
